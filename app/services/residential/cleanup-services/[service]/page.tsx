...
export const generateMetadata = ({ params }: { params: { service: string } }) => {
  const service = getServiceBySlug("residential", "cleanup-services", params.service);
  if (!service) return {};
  return buildMetadata({
    title: service.metaTitle,
    description: service.metaDescription,
    path: `/services/residential/cleanup-services/${params.service}/`,
  });
};
...
export default function ServiceDetailPage({ params }: { params: { service: string } }) {
  ...
  const crumbs = [
    ...
    {
      name: service.title,
      url: `https://flood.repair/services/residential/cleanup-services/${params.service}/`,
    },
  ];
