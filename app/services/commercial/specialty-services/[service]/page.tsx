...
export const generateMetadata = ({ params }: { params: { service: string } }) => {
  const service = getServiceBySlug("commercial", "specialty-services", params.service);
  if (!service) return {};
  return buildMetadata({
    title: service.metaTitle,
    description: service.metaDescription,
    path: `/services/commercial/specialty-services/${params.service}/`,
  });
};
...
export default function ServiceDetailPage({ params }: { params: { service: string } }) {
  ...
  const crumbs = [
    ...
    {
      name: service.title,
      url: `https://flood.repair/services/commercial/specialty-services/${params.service}/`,
    },
  ];
